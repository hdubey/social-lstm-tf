    ...
    def __init__(self, args, infer=False):
        ...
        with tf.variable_scope("Hidden_states"):
            ...
            self.output_states = tf.split(
                tf.zeros([args.maxNumPeds, cell.output_size]),
                args.maxNumPeds, 0)
        ...
        with tf.name_scope("frame_data_tensors"):
            ...
            frame_data = [tf.squeeze(input_, [0]) for input_ in tf.split(
                self.input_data, args.seq_length, 0)]
        ...
        with tf.name_scope("Distribution_parameters_stuff"):
            ...
            self.initial_output = tf.split(
                tf.zeros([args.maxNumPeds, self.output_size]),
                args.maxNumPeds, 0)
            ...
            for ped in range(args.maxNumPeds):
                ...
                with tf.name_scope("extract_target_ped"):
                    ...
                    [x_data, y_data] = tf.split(
                        tf.slice(frame_target_data[seq], [ped, 1], [1, 2]),
                        2, 1)
